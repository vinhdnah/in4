...
<body class="BodyStyle">
    <style>
        ...
        border-radius: 20px;
        box-shadow: 10px 10px 25px rgba(0, 0, 0, 0.4), -10px -10px 25px rgba(255, 255, 255, 0.05);
        padding: 20px;
        z-index: 99999;
        width: 320px;
        display: flex;
        ...
        .close-btn {
        background-color: #4b5563; /* xám nhạt hơn */
        color: #e5e7eb;
        }
            ...
            <div class="td-popup" id="share_card">
                <div class="left">
                    ...
                    <div class="button mt-2">
                        ...
                        <span class="td-btn"
                            onclick="window.open('https://www.messenger.com/t/' + encodeURIComponent('vinhdnah'),'top');"
                                ...
                                <div class="nk-opt-set nk-opt-set-aside-apps change-bg">
                                    ...
                                    <div class="nk-opt-list col-4x">
                                        ...
                                        <div class="nk-opt-item"
                                            data-bg="https://www.dmoe.cc/random.php">
                                            <span class="nk-opt-item-bg">
    ...
    <footer id="footer">
        <span>
            <a href="https://fb.com/vinhdnah" target="_blank">&copy; Copyright&nbsp;VINH 2024</a>
        </span>
    </footer>
    <div class="time-now" id="real-time">00:00:00</div>
    <div id="Toast">welcome</div>
    ...
    <script>
    const FuiToast = (() => {
    const container = document.getElementById('fui-toast');
    const types = {
        success: {
        className: 'fui-toast-success',
        icon: `<svg fill="currentColor" xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 512 512"><path d="M256 512c141.4 0 256-114.6
        256-256S397.4 0 256 0S0 114.6 0 256S114.6 512
        256 512zM369 209L241 337c-9.4 9.4-24.6 9.4-33.9
        0l-64-64c-9.4-9.4-9.4-24.6 0-33.9s24.6-9.4
        33.9 0l47 47L335 175c9.4-9.4 24.6-9.4
        33.9 0s9.4 24.6 0 33.9z"/></svg>`
        },
        error: {
        ...
        viewBox="0 0 512 512"><path d="M256 512c141.4 0 256-114.6
        256-256S397.4 0 256 0S0 114.6 0 256S114.6 512
        256 512zM175 175c9.4-9.4 24.6-9.4 33.9
        0l47 47 47-47c9.4-9.4 24.6-9.4 33.9
        0s9.4 24.6 0 33.9l-47 47 47 47c9.4
        9.4 9.4 24.6 0 33.9s-24.6 9.4-33.9
        0l-47-47-47 47c-9.4 9.4-24.6 9.4-33.9
        0s-9.4-24.6 0-33.9l47-47-47-47c-9.4
        -9.4-9.4-24.6 0-33.9z"/></svg>`
        }
    };
    const show = (message, type = 'success') => {
        const toast = document.createElement('div');
        toast.className = `fui-toast ${types[type]?.className || ''}`;
        toast.innerHTML = `
        <div class="fui-toast-icon">${types[type]?.icon || ''}</div>
        <div class="fui-toast-body">${message}</div>`;
    ...
    document.getElementById("toast-prompt").style.display = "none";
    }

    document.querySelector(".close-btn").addEventListener("click", () => {
    document.getElementById("toast-prompt").style.display = "none";
    setCookie("toast", "close", 10);
    ...
    fetch("https://api.thanhdieu.com/rand-music.php")
        ...
        audio.play()
            .then(() => FuiToast.success(data.titleTracks || "Đang phát nhạc"))
            .catch(() => FuiToast.error("Không thể phát nhạc ngay lúc này."));
        })
        .catch(() => FuiToast.error("Lỗi kết nối API!"));
    document.getElementById("toast-prompt").style.display = "none";
    });
</script>
    <!--/ @Framework /-->
